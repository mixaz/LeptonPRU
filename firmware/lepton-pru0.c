...
static uint16_t count_10msec = 0;
static uint8_t count_10msec_0 = 0;
...
static int handle_command(uint32_t cmd) {
    switch (cmd) {
        ...
        case CMD_CONFIGURE:
            ...
            if(cxt.frames_in_file == 0) {
              cxt.frames_in_file = FRAMES_PER_FILE;
            }
            init_pwm();
            return 0;
    }
    ...
    while (1) {
        ...
        if (state_run == 1) {
          if (samples_count >= BUFFER_SIZE) {
            ...
            if (cxt.frames_received-cxt.frames_dropped >= cxt.frames_in_file) {
              state_run = 2;
              continue;
            }
          }
